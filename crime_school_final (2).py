...
# In[32]:
value = int(input('Enter school ID:'))
...
writer = csv.writer(f)
writer.writerow(['Primary_Type','Student_Growth_Rating','Teacher_Attendance_Year_1_Pct','Student_Attendance_Year_1_Pct','School_Survey_Teacher_Response_Rate_Pct',
            'One_Year_Dropout_Rate_Avg_Pct','School_Survey_Student_Response_Rate_Pct','Suspensions_Per_100_Students_Avg_Pct',
            'Misconducts_To_Suspensions_Avg_Pct','Distance'])
deglen = 110.25
# train_distances = {}
var1 =0
for i in range(len(train_schools)):
    if train_schools['School_ID'][i] == value:
    ...
    #print train_distance
        # row = []
            ...
            if train_distance < 3: 
                ...
                writer.writerow([train_crimes['Primary_Type'][j], var1, train_schools['Teacher_Attendance_Year_1_Pct'][i],
                train_schools['Student_Attendance_Year_1_Pct'][i], train_schools['School_Survey_Teacher_Response_Rate_Pct'][i],
                train_schools['One_Year_Dropout_Rate_Avg_Pct'][i], train_schools['School_Survey_Student_Response_Rate_Pct'][i],
                train_schools['Suspensions_Per_100_Students_Avg_Pct'][i], train_schools['Misconducts_To_Suspensions_Avg_Pct'][i],
                train_distance])
